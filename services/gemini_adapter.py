...
class GeminiAdapter(BaseAIAdapter):
    ...
    def chat_completion(
        ...
        **kwargs
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                for tool_call in msg["tool_calls"]:
                    ...
                    # Gemini returns this opaque signature on function-call
                    # parts generated by thinking models.  It must accompany
                    # the same part when conversation history is replayed.
                    if thought_signature := tool_call.get("thought_signature"):
        ...
        # Thought summaries are model-internal context, not assistant output.
        output_text = "".join(
        ...
        if tool_calls:
            # Match Odoo's native Gemini behavior: intermediate text that
            # accompanies a function call is context, not a final user reply.
            output_text = ""
        finish_reason = first_cand.get("finishReason")

        # Replaying the complete model content is required by Gemini's
        # stateless generateContent API.  This preserves signatures on all
        # parts and also keeps parallel function calls in their original order.
        provider_history = []
        if tool_calls:
            model_content = dict(content_obj)
            model_content.setdefault("role", "model")
            provider_history.append(model_content)
